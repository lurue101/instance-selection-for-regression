...
class FindSubset_Vect_No_ValLoss(object):
    ...
    def precompute(self, f_pi_epoch, p_epoch, alphas):
        ...
        loader_tr = DataLoader(
            CustomDataset_WithId(self.x_trn, self.y_trn, transform=None),
            shuffle=False,
        ...
        for batch_idx in list(loader_tr.batch_sampler):
            ...
            exten_inp = torch.cat(
                (inputs, torch.ones(inputs.shape[0], device=self.device).view(-1, 1)),
                dim=1,
            )
            ...
            for batch_idx_val in list(loader_val.batch_sampler):
                ...
                exten_val = torch.cat(
                    (
                        inputs_val,
                        torch.ones(inputs_val.shape[0], device=self.device).view(-1, 1),
                    ),
                    dim=1,
                )

                exten_val_y = torch.mean(targets_val).repeat(
                    min(self.batch_size * 20, targets.shape[0])
                )

                val_loss = (
                    torch.sum(weights * torch.mean(exten_val, dim=0), dim=1)
                    - exten_val_y
                )
            ...
            self.F_values[idxs] = (
                ...
                + self.lam * reg
                + torch.max(
                    torch.zeros_like(ele_alphas),
                    (val_losses / len(loader_val.batch_sampler) - ele_delta)
                    * ele_alphas,
                )
            )
    ...
    def return_subset(
        ...
        for batch_idx in list(loader_tr.batch_sampler):
            ...
            exp_avg_w = w_exp_avg.repeat(
                targets.shape[0], 1
            )  # torch.zeros_like(weights)
            exp_avg_sq_w = w_exp_avg_sq.repeat(
                targets.shape[0], 1
            )  # torch.zeros_like(weights)

            exp_avg_a = a_exp_avg.repeat(
                targets.shape[0]
            )  # torch.zeros_like(ele_alphas)
            exp_avg_sq_a = a_exp_avg_sq.repeat(
                targets.shape[0]
            )  # torch.zeros_like(ele_alphas)
            ...
            for i in range(p_epoch):
                ...
                for batch_idx_val in list(loader_val.batch_sampler):
                    ...
                    exten_val = torch.cat(
                        (
                            inputs_val,
                            torch.ones(inputs_val.shape[0], device=self.device).view(
                                -1, 1
                            ),
                        ),
                        dim=1,
                    ...
                    exten_val_y = (
                        targets_val.view(-1, 1)
                        .repeat(1, min(self.batch_size, targets.shape[0]))
                        .to(device_new)
                    )
                    # print(exten_val_y[0])
                    ...
                    val_loss_p = 2 * (
                        torch.matmul(
                            exten_val, torch.transpose(weights, 0, 1).to(device_new)
                        )
                        - exten_val_y
                    )
                    ...
                    # print(mod_val[0])
                    fin_val_loss_g += torch.mean(
                        val_loss_p[:, :, None] * exten_val[:, None, :], dim=0
                    )
                ...
                for batch_idx_trn in list(loader_tr.batch_sampler):
                    ...
                    exten_trn = torch.cat(
                        (
                            inputs_trn,
                            torch.ones(inputs_trn.shape[0], device=self.device).view(
                                -1, 1
                            ),
                        ),
                        dim=1,
                    ...
                    sum_trn_loss_p = 2 * (
                        torch.matmul(
                            exten_trn, torch.transpose(weights, 0, 1).to(device_new)
                        )
                        - exten_trn_y
                    )
                    ...
                    sum_fin_trn_loss_g += torch.sum(
                        sum_trn_loss_p[:, :, None] * exten_trn[:, None, :], dim=0
                    )
            ...
            for batch_idx_val in list(loader_val.batch_sampler):
                ...
                val_loss = (
                    torch.matmul(exten_val, torch.transpose(weights, 0, 1))
                    - exten_val_y
                )
            ...
            for batch_idx_trn in list(loader_tr.batch_sampler):
                ...
                exten_trn = torch.cat(
                    (
                        inputs_trn,
                        torch.ones(inputs_trn.shape[0], device=self.device).view(-1, 1),
                    ),
                    dim=1,
                )
                exten_trn_y = targets_trn.view(-1, 1).repeat(
                    1, min(self.batch_size, targets.shape[0])
                )
                # print(exten_val_y[0])

                trn_loss = (
                    torch.matmul(exten_trn, torch.transpose(weights, 0, 1))
                    - exten_trn_y
                )
            ...
            m_values[
                torch.tensor(curr_subset, dtype=torch.long)[
                    b_idxs * self.batch_size : (b_idxs + 1) * self.batch_size
                ]
            ] = abs_value
...
class FindSubset_Vect_TrnLoss(object):
    ...
    def precompute(self, f_pi_epoch, p_epoch, alphas):
        ...
        # alphas.requires_grad = False
        loader_val = DataLoader(
            CustomDataset(self.x_val, self.y_val, transform=None),
            shuffle=False,
            batch_size=self.batch_size,
        )
    ...
    def return_subset(
        ...
        with torch.no_grad():
            ...
            F_curr += (
                self.lam * l2_reg * len(curr_subset)
        ...
        for batch_idx in list(loader_tr.batch_sampler):
            ...
            for i in range(p_epoch):
                ...
                weight_grad = fin_trn_loss_g + 2 * rem_len * torch.cat(
                    (
                        weights[:, :-1],
                        torch.zeros((weights.shape[0], 1), device=self.device),
                    ),
