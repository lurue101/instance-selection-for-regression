    ...
    if data_name == "Community_Crime_old":
        ...
        for i in protect_feature:
            ...
            for cl in classes[:-1]:
                ...
                indices.extend(
                    list(np.random.choice(idx, size=min(N, curr_N), replace=False))
                )
                ...
                val_data_slices.append(
                    torch.from_numpy(data[indices]).float().to(device)
                )
                val_label_slices.append(
                    torch.from_numpy(labels[indices]).float().to(device)
                )

                tst_data_slices.append(
                    torch.from_numpy(data[indices_tst]).float().to(device)
                )
                tst_label_slices.append(
                    torch.from_numpy(labels[indices_tst]).float().to(device)
                )
            ...
            idx = list(idxs)
            indices_tst.extend(
                list(np.random.choice(idx, size=N + count, replace=False))
            )
            total_set.difference(indices_tst)
        ...
        for pf in protect_feature:
            ...
            idx = list(idxs)
            indices_tst = list(
                np.random.choice(idx, size=min(N, curr_N), replace=False)
            )
            total_set.difference(indices_tst)
        ...
        for j in range(len(val_data_slices)):
            ...
            val_label_slices[j] = (
                torch.from_numpy(
                    np.reshape(
                        sc_l.transform(np.reshape(val_label_slices[j], (-1, 1))), (-1)
                    )
                )
                .float()
                .to(device)
            ...
            tst_label_slices[j] = (
                torch.from_numpy(
                    np.reshape(
                        sc_l.transform(np.reshape(tst_label_slices[j], (-1, 1))), (-1)
                    )
                )
                .float()
                .to(device)
