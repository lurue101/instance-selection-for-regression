...
def house_price_load(trn_path, tst_path, save_data=False):
    ...
    train_10_percent_missing_features = (
        train_clean.isnull()
        .sum()
        .sort_values(ascending=False)
        .loc[lambda x: (x < 0.10 * 1460) & (x != 0)]
        .index.to_list()
    )
    train_10_percent_missing_features_cat = (
        train_clean[train_10_percent_missing_features]
        .select_dtypes("object")
        .columns.to_list()
    )
    train_10_percent_missing_features_num = (
        train_clean[train_10_percent_missing_features]
        .select_dtypes("number")
        .columns.to_list()
    )

    train_clean[train_10_percent_missing_features_cat] = train_clean[
        train_10_percent_missing_features_cat
    ].fillna(train_clean[train_10_percent_missing_features_cat].mode().iloc[0])
    train_clean[train_10_percent_missing_features_num] = train_clean[
        train_10_percent_missing_features_num
    ].fillna(train_clean[train_10_percent_missing_features_num].median().iloc[0])

    test_10_percent_missing_features = (
        test_clean.isnull()
        .sum()
        .sort_values(ascending=False)
        .loc[lambda x: (x < 0.10 * 1460) & (x != 0)]
    ...
    test_10_percent_missing_features_cat = (
        test_clean[test_10_percent_missing_features]
        .select_dtypes("object")
        .columns.to_list()
    )
    test_10_percent_missing_features_num = (
        test_clean[test_10_percent_missing_features]
        .select_dtypes("number")
        .columns.to_list()
    )

    test_clean[test_10_percent_missing_features_cat] = test_clean[
        test_10_percent_missing_features_cat
    ].fillna(test_clean[test_10_percent_missing_features_cat].mode().iloc[0])
    test_clean[test_10_percent_missing_features_num] = test_clean[
        test_10_percent_missing_features_num
    ].fillna(test_clean[test_10_percent_missing_features_num].median().iloc[0])

    train_clean["LotFrontage"] = train_clean["LotFrontage"].fillna(
        train_clean["LotFrontage"].median()
    )
    test_clean["LotFrontage"] = test_clean["LotFrontage"].fillna(
        test_clean["LotFrontage"].median()
    )
    ...
    X_total_encoded = MultiColumnLabelEncoder(columns=cat_features).fit_transform(
        X_total
    )

    X_train_clean_encoded = X_total_encoded.iloc[X_train_clean_index, :]
    X_test_clean_encoded = X_total_encoded.iloc[X_test_clean_index, :].reset_index(
        drop=True
    )
